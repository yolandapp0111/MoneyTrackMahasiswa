...
import { ArrowDownCircle, ArrowUpCircle, Pencil, Trash2, Loader2, X } from 'lucide-react';
...
const TransaksiListView: React.FC<Props> = ({ transaksi, jenis, onRefresh }) => {
  const [editingData, setEditingData] = useState<any>(null);
  const [isSaving, setIsSaving] = useState(false);

  const filtered = transaksi.filter(t => t.jenis === jenis);
  const total = filtered.reduce((s, t) => s + parseFloat(t.jumlah), 0);
  const warna = jenis === 'pemasukan' ? '#1F8B4C' : '#FA5252';
  const bgWarna = jenis === 'pemasukan' ? '#E8F5E9' : '#FEE2E2';
  const Icon = jenis === 'pemasukan' ? ArrowDownCircle : ArrowUpCircle;
  ...
  return (
    <div>
      ...
      {/* Summary Card */}
      <div className="card mb-5" style={{ background: `linear-gradient(135deg, ${warna}, ${jenis === 'pemasukan' ? '#34D399' : '#F87171'})`, color: '#fff', padding: '1.5rem 2rem' }}>
        <p style={{ fontSize: '0.85rem', opacity: 0.9, marginBottom: '0.4rem' }}>Total {jenis === 'pemasukan' ? 'Pemasukan' : 'Pengeluaran'}</p>
        <h2 style={{ fontSize: '2rem', fontWeight: 700, marginBottom: '0.3rem' }}>{formatRp(total)}</h2>
        <p style={{ fontSize: '0.8rem', opacity: 0.8 }}>{filtered.length} transaksi tercatat</p>
      </div>

      {/* List */}
      <div className="card">
        <h3 style={{ fontWeight: 700, marginBottom: '1.25rem' }}>
          Daftar {jenis === 'pemasukan' ? 'Pemasukan' : 'Pengeluaran'}
        </h3>
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '3rem 0', color: '#6C757D' }}>
            <div style={{ fontSize: '3rem', marginBottom: '0.75rem' }}>{jenis === 'pemasukan' ? '💰' : '🛒'}</div>
            <p style={{ fontWeight: 600, marginBottom: '0.25rem' }}>Belum ada data {jenis}</p>
            <p style={{ fontSize: '0.85rem' }}>Mulai catat {jenis} pertamamu!</p>
          </div>
        )}
        {filtered.map((t, i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 0', borderBottom: i < filtered.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
              <div style={{ background: bgWarna, borderRadius: 10, padding: 10, color: warna }}>
                <Icon size={20} />
              </div>
              <div>
                <p style={{ fontWeight: 600, fontSize: '0.9rem' }}>{t.kategori}</p>
                <p style={{ fontSize: '0.75rem', color: '#6C757D' }}>
                  {new Date(t.tanggal).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
                {t.catatan && <p style={{ fontSize: '0.75rem', color: '#9CA3AF', marginTop: 2 }}>{t.catatan}</p>}
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <span style={{ fontWeight: 700, color: warna, fontSize: '0.95rem' }}>
                {jenis === 'pemasukan' ? '+' : '-'} {formatRp(parseFloat(t.jumlah))}
              </span>
              <div style={{ display: 'flex', gap: '0.35rem' }}>
                <button onClick={() => setEditingData(t)} style={{ background: '#E0F2FE', color: '#0284C7', border: 'none', borderRadius: 6, padding: '0.4rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Edit">
                  <Pencil size={14} />
                </button>
                <button onClick={() => handleDelete(t.id)} style={{ background: '#FEE2E2', color: '#E11D48', border: 'none', borderRadius: 6, padding: '0.4rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Hapus">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
...
export default TransaksiListView;
